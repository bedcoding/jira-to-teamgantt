...
export function upsertTgTasks(incoming) {
...
// 단일 TG 작업의 일부 필드만 로컬 갱신(서버 PATCH 후 화면에 즉시 반영용).
export async function patchTgTask(id, fields) {
  const { tgTasks = {} } = await chrome.storage.local.get("tgTasks");
  const key = String(id);
  if (!tgTasks[key]) return;
  tgTasks[key] = { ...tgTasks[key], ...fields };
  await chrome.storage.local.set({ tgTasks });
}
