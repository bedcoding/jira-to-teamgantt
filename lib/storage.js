...
// Jira/TG 데이터가 사라지면 비교/동기화의 기반이 사라지므로 syncQueue + manualChecked 도 같이 정리.
// 안 그러면 '완료 N건' 같은 진행 기록만 남아 어색해 보임.
export async function clearJiraIssues() {
  await chrome.storage.local.remove(["jiraIssues", "syncQueue", "manualChecked"]);
}
...
export async function upsertTgTasks(incoming) {
  // incoming 은 정규화된 [{id, rawTitle, jiraKey, start, end, progress, assignees}] 배열
...
// fetch 가로채기 응답(`/v1/projects/{id}/children?is_flat_list=true`)을
// 우리 표준 task 객체로 변환. 응답은 평탄화된 배열 또는 { children: [...] } 형태.
//
// 응답 구조 메모:
// - type: "group" | "subgroup" | "task" | "milestone"  → task 만 사용
// - resources[]: 각 항목의 type_id 가 우리가 비교해야 할 person ID (id 는 resource-link 고유 ID)
export function normalizeTgFromFetch(payload, opts = {}) {
  ...
  for (const it of items) {
    ...
    // task 만 통과. group/subgroup/milestone 은 제외.
    ...
    // 내 task 만 필터. myTgId 가 주어졌으면, resources 가 비었거나
    // 매칭 안 되는 task 는 모두 제외.
    if (myTgId) {
...
//   pendingKey: 방금 TG 입력창에 박은 항목의 key. 사용자가 Enter 로 저장하면 다음 단축키에서 doneKeys 로 승격됨.
