...
// 이슈 한 건씩 제거. upsertJiraIssues는 누적 전용(응답에 없는 키를 지우는 prune이 없음)이라
// Jira에서 이슈를 지우거나 JQL을 잘못 걸어 들어온 건은 재수집으로는 사라지지 않는다.
// jiraIssues만 건드리고 manualChecked/apiSelectedKeys/apiCreatedKeys는 남긴다:
//   - 비교 탭은 jiraIssues를 기준으로 행을 그리므로 남은 키는 화면에 나타나지 않는다.
//   - 되돌리기를 원본 그대로 복원할 수 있어야 하는데, 진행 기록까지 지우면 복원이 반쪽이 된다.
//   - 특히 apiCreatedKeys는 'TeamGantt에 실제로 만들었다'는 이력이라, 로컬 목록에서
//     행을 지웠다는 사실만으로 해제하면 같은 이슈가 TG에 중복 생성된다.
//     (TG에서 지운 경우는 upsertTgTasks의 prune이 removeApiCreated로 알아서 해제한다.)
// 반환: 삭제된 레코드 배열. 되돌리기(restoreJiraIssues)에 그대로 넘긴다.
export async function removeJiraIssues(keys) {
  const { jiraIssues = {} } = await chrome.storage.local.get("jiraIssues");
  const removed = [];
  for (const key of keys) {
    const rec = jiraIssues[key];
    if (!rec) continue;
    removed.push(rec);
    delete jiraIssues[key];
  }
  if (removed.length) await chrome.storage.local.set({ jiraIssues });
  return removed;
}

// removeJiraIssues 되돌리기. hash/collectedAt까지 원본 그대로 꽂아야 다음 수집에서
// '갱신'으로 잡히지 않는다. upsertJiraIssues로 다시 넣으면 레코드에 이미 있는 hash가
// rest에 섞여 들어가 hash 계산이 어긋난다.
export async function restoreJiraIssues(records) {
  const { jiraIssues = {} } = await chrome.storage.local.get("jiraIssues");
  for (const rec of records) {
    if (!rec?.key) continue;
    jiraIssues[rec.key] = rec;
  }
  await chrome.storage.local.set({ jiraIssues });
  return Object.keys(jiraIssues).length;
}
